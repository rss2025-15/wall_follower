...
class WallFollower(Node):
    ...
    def callback(self, lidar_msg):
        ...
        if sliced_angles.any():
            ...
            # distance = abs(c) / np.sqrt(1 + m**2)
            # error = (distance - self.DESIRED_DISTANCE) * self.SIDE
            self.total+=error**2
            self.abs_e += abs(error)
            start, stop = int(((0.45)*np.pi - lidar_msg.angle_min)/lidar_msg.angle_increment), int(((0.55)*np.pi - lidar_msg.angle_min)/lidar_msg.angle_increment)
            self.get_logger().info(f"{start} {stop} {str(lidar_msg.ranges[start:start+1])}")
            self.lidar_error+=abs(self.DESIRED_DISTANCE-np.mean(np.array(lidar_msg.ranges[start:stop])))
        ...
        self.counter+=1
        self.get_logger().info(f"mse: {(self.total/self.counter)**0.5}, abs e: {self.abs_e/self.counter} lidar errror:{self.lidar_error/self.counter} count: {self.counter}")
